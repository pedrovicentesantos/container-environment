...
describe('redis-cache test', () => {
  ...
  describe('#put test', () => {
    context('When given a key and a value', () => {
      it('should set the key value entry in redis without expiration', async () => {
    ...
    context('When given a key, a value and a positive expiration time', () => {
      it('should set the key value entry in redis with expiration', async () => {
        const cache = Cache({ redis });
        const key = faker.lorem.word();
        const value = faker.lorem.word();
        const expiration = faker.datatype.number({ min: 60, max: 120, precision: 1 });
  ...
  describe('#get test', () => {
    ...
    context('When given a key that was set', () => {
      it('should resolve its value', async () => {
        const cache = Cache({ redis });
        const key = faker.lorem.word();
        const value = faker.lorem.word();
